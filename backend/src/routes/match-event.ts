...
// POST /matches/:id/events — crear evento (gol/amarilla/roja)
router.post('/:id/events', requireAuth, requireRole(['admin']), async (req, res) => {
  try {
    ...
    const playerIdNum = playerId === undefined || playerId === null ? null : Number(playerId);
    const minuteNum = minute === undefined || minute === null ? 0 : Number(minute);
    ...
    if (!Number.isFinite(minuteNum) || minuteNum < 0 || minuteNum > 130) {
      return res.status(400).json({ error: 'minute inválido (0-130)' });
    }
    ...
    if (playerIdNum) {
      ...
      if (player.teamId !== teamIdNum) return res.status(400).json({ error: 'Jugador no pertenece a ese equipo' });
    }
    ...
    const event = await prisma.matchEvent.create({
      data: {
        matchId,
        teamId: teamIdNum,
        playerId: playerIdNum ?? undefined,
        type,
        minute: minuteNum,
      },
    });
    ...
    const updated = await prisma.match.findUnique({
      ...
      include: {
        ...
        awayTeam: true,
        venue: true,
        events: { include: { player: true, team: true }, orderBy: { minute: 'asc' } },
      },
    });
...
// DELETE /matches/:id/events/:eventId — eliminar evento
router.delete('/:id/events/:eventId', requireAuth, requireRole(['admin']), async (req, res) => {
  try {
    const matchId = Number(req.params.id);
    const eventId = Number(req.params.eventId);
    if (!Number.isFinite(matchId) || !Number.isFinite(eventId)) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (event.type === 'goal') {
        ...
        const homeGoals = goals.find((g) => g.teamId === match.homeTeamId)?._count._all ?? 0;
        const awayGoals = goals.find((g) => g.teamId === match.awayTeamId)?._count._all ?? 0;
